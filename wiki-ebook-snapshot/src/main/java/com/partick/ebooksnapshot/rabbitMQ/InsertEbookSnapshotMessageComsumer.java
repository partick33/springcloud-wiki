...
@Service
public class InsertEbookSnapshotMessageComsumer {
    @Autowired
    ...
    private static final Logger LOG = LoggerFactory.getLogger(InsertEbookSnapshotMessageComsumer.class);

    private final static String EXCHANGE = "wiki.ebookSnapshot";

    @RabbitListener(
            bindings = @QueueBinding(value = @Queue(value = "wiki.insertEbook", autoDelete = "true"),
                        exchange = @Exchange(value = EXCHANGE,type = ExchangeTypes.TOPIC),
                        key = "insert.ebookSnapshot"
            )
    )
    @RabbitHandler
    public void insertEbookMessageReceived(HashMap map) {
        String logId = (String) map.get("LOG_ID");
        MDC.put("LOG_ID", logId);
        LOG.info("RabbitMQ收到消息：{}", map);
        int rows = ebookSnapshotService.insertEbookSnapshot();
        LOG.info("插入新的电子书快照：{}本，书名为：{}", rows, map.get("ebookName"));
    }
}
